  ...
  return (
    <section aria-labelledby={`${inputId}-title`} className={`${cardClass} p-4`}>
      <div className="flex items-start gap-3"><Paperclip className="mt-0.5 size-5 text-emerald-700" /><div>
        ...
        <p className="text-xs text-slate-500">{transactionName} · JPEG, PNG, WebP, atau PDF · maksimum 10 MiB</p>
      </div></div>
      ...
      <div className="mt-3 border-t border-slate-100 pt-3 dark:border-slate-800">
        {loading ? <p role="status" className="flex items-center gap-2 text-sm text-slate-500"><LoaderCircle className="size-4 animate-spin" />Memuat lampiran...</p>
          : attachments.length === 0 ? <p className="text-sm text-slate-500">Belum ada lampiran untuk transaksi ini.</p>
            : <ul aria-label="Daftar lampiran" className="divide-y divide-slate-100 dark:divide-slate-800">{attachments.map((attachment) => <li key={attachment.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center">
              <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{attachment.original_filename}</p><p className="text-xs text-slate-500">{displayType(attachment.content_type)} · {formatBytes(Number(attachment.size_bytes))}</p></div>
