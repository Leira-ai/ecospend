...
export function ReportsDashboard() {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="Ringkasan data" title="Laporan" description="Setiap ringkasan, tren, tabel, dan ekspor berasal dari transaksi periode terkait." actions={<button type="button" onClick={() => window.print()} className={buttonSecondary}><Printer className="size-4" />Cetak</button>} />
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[{ label: "Pemasukan", value: formatRupiah(income) }, { label: "Pengeluaran", value: formatRupiah(expense) }, { label: "Saldo bersih", value: formatRupiah(income - expense) }, { label: "Estimasi karbon", value: `${formatNumber(carbon)} kg CO₂e` }].map((item) => <Card key={item.label} className="p-5"><p className="text-sm text-slate-600">{item.label}</p><p className="mt-2 text-2xl font-bold">{item.value}</p></Card>)}</div>
    <Card className="p-5 sm:p-6"><h2 className="font-bold">Perbandingan arus kas</h2><p className="text-xs text-slate-600">Enam bulan sampai periode terpilih · dari transaksi</p><CashflowChart data={trend} /></Card>
    <Card className="overflow-hidden"><div className="border-b border-slate-100 p-5 dark:border-slate-800"><h2 className="font-bold">Rincian periode</h2><p className="text-xs text-slate-600">{rows.length} transaksi tercatat</p></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 dark:bg-slate-800"><tr>{["Tanggal", "Nama", "Kategori", "Jenis", "Nominal", "Karbon"].map((header) => <th key={header} className="px-4 py-3">{header}</th>)}</tr></thead><tbody>{rows.map((item) => <tr key={item.id} className="border-t border-slate-100 dark:border-slate-800"><td className="px-4 py-3">{item.date}</td><td className="px-4 py-3 font-medium">{item.name}</td><td className="px-4 py-3">{item.category}</td><td className="px-4 py-3 capitalize">{item.type}</td><td className="px-4 py-3 font-semibold">{formatRupiah(item.amount)}</td><td className="px-4 py-3">{formatNumber(item.type === "pengeluaran" ? item.carbonKg : 0)} kg</td></tr>)}</tbody></table></div></Card>
    <Card className="p-5 print:hidden"><h2 className="font-bold">Unduh laporan</h2><p className="mt-1 text-sm text-slate-600">File hanya memuat periode terpilih; CSV dan XLSX menetralkan awalan formula.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={csv} className={buttonSecondary}><Download className="size-4" />CSV</button><button type="button" onClick={xlsx} className={buttonSecondary}><FileSpreadsheet className="size-4" />XLSX</button><button type="button" onClick={json} className={buttonSecondary}><FileJson className="size-4" />JSON</button><button type="button" onClick={() => window.print()} className={buttonSecondary}><Printer className="size-4" />Cetak / PDF</button></div></Card>
  </div>;
}
