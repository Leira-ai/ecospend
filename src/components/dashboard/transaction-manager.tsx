  ...
  return (
    <>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari transaksi, kategori, atau akun" aria-label="Cari transaksi" className={`${inputClass} pl-10`} /></div>
      ...
      <div className={`${cardClass} overflow-hidden`}>
        {filtered.length === 0 ? <div className="p-5"><EmptyState title="Transaksi tidak ditemukan" description="Ubah kata kunci atau filter untuk melihat transaksi lain." /></div> : <>
          <div className="hidden overflow-x-auto md:block"><table className="w-full border-collapse text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800/60 dark:text-slate-400"><tr><th className="w-12 px-4 py-3"><input type="checkbox" aria-label="Pilih semua transaksi di halaman" checked={allVisible} onChange={() => setSelected((current) => allVisible ? current.filter((id) => !visibleIds.includes(id)) : [...new Set([...current, ...visibleIds])])} className="size-4 accent-emerald-700" /></th>{table.getHeaderGroups()[0]?.headers.map((header) => <th key={header.id} className={`px-4 py-3 ${header.id === "amount" ? "text-right" : ""}`}><button type="button" onClick={header.column.getToggleSortingHandler()} className="inline-flex items-center gap-1 font-semibold hover:text-emerald-700"><table.FlexRender header={header} />{header.column.getIsSorted() && <ArrowDownAZ className={`size-3 ${header.column.getIsSorted() === "desc" ? "rotate-180" : ""}`} />}</button></th>)}<th className="w-24 px-4 py-3"><span className="sr-only">Aksi</span></th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{table.getRowModel().rows.map((row) => { const item = row.original; return <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40"><td className="px-4 py-3"><input type="checkbox" aria-label={`Pilih ${item.name}`} checked={selected.includes(item.id)} onChange={() => toggle(item.id)} className="size-4 accent-emerald-700" /></td><td className="whitespace-nowrap px-4 py-3 text-slate-500">{formatDate(item.date)}</td><td className="max-w-56 px-4 py-3"><p className="truncate font-semibold text-slate-900 dark:text-white">{item.name}</p>{item.notes && <p className="truncate text-xs text-slate-500">{item.notes}</p>}</td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs dark:bg-slate-800">{item.category}</span></td><td className="px-4 py-3 text-slate-600 dark:text-slate-300">{item.account}{item.destinationAccount && ` → ${item.destinationAccount}`}</td><td className="px-4 py-3 capitalize">{item.type}</td><td className={`whitespace-nowrap px-4 py-3 text-right font-bold ${item.type === "pemasukan" ? "text-emerald-700" : ""}`}>{item.type === "pemasukan" ? "+" : item.type === "pengeluaran" ? "−" : ""}{formatRupiah(item.amount)}</td><td className="px-4 py-3"><div className="flex justify-end"><button type="button" onClick={() => openEdit(item)} aria-label={`Edit ${item.name}`} className="grid size-9 place-items-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"><Pencil className="size-4" /></button>{!store.isDemo && item.type !== "transfer" && <button type="button" onClick={() => setAttachmentTransaction(item)} aria-label={`Kelola lampiran ${item.name}`} className="grid size-9 place-items-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"><Paperclip className="size-4" /></button>}<button type="button" onClick={() => setDeleting([item.id])} aria-label={`Hapus ${item.name}`} className="grid size-9 place-items-center rounded-lg text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950"><Trash2 className="size-4" /></button></div></td></tr>; })}</tbody></table></div>
          <div className="divide-y divide-slate-100 md:hidden dark:divide-slate-800">{table.getRowModel().rows.map((row) => { const item = row.original; return <article key={item.id} className="p-4"><div className="flex items-start gap-3"><input type="checkbox" aria-label={`Pilih ${item.name}`} checked={selected.includes(item.id)} onChange={() => toggle(item.id)} className="mt-1 size-4 accent-emerald-700" /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold text-slate-900 dark:text-white">{item.name}</h3><p className="text-xs text-slate-500">{formatDate(item.date)} · {item.account}</p></div><p className={`whitespace-nowrap font-bold ${item.type === "pemasukan" ? "text-emerald-700" : ""}`}>{item.type === "pemasukan" ? "+" : item.type === "pengeluaran" ? "−" : ""}{formatRupiah(item.amount)}</p></div><div className="mt-3 flex items-center gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs dark:bg-slate-800">{item.category}</span><button type="button" onClick={() => openEdit(item)} className="ml-auto text-xs font-semibold text-emerald-700">Edit</button>{!store.isDemo && item.type !== "transfer" && <button type="button" onClick={() => setAttachmentTransaction(item)} className="text-xs font-semibold text-emerald-700">Lampiran</button>}<button type="button" onClick={() => setDeleting([item.id])} className="text-xs font-semibold text-rose-600">Hapus</button></div></div></div></article>; })}</div>
          <div className="flex flex-col gap-3 border-t border-slate-100 px-4 py-3 text-sm sm:flex-row sm:items-center dark:border-slate-800"><p className="text-slate-500">{filtered.length} transaksi · Halaman {table.state.pagination.pageIndex + 1} dari {table.getPageCount()}</p><div className="ml-auto flex items-center gap-2"><button type="button" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className={buttonSecondary}><ChevronLeft className="size-4" /><span className="sr-only sm:not-sr-only">Sebelumnya</span></button><button type="button" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className={buttonSecondary}><span className="sr-only sm:not-sr-only">Berikutnya</span><ChevronRight className="size-4" /></button></div></div>
        </>}
      </div>
