...
function Brand({ isDemo }: { isDemo: boolean }) {
  return <Link href={dashboardHref("/dashboard", isDemo)} className="group flex min-h-11 items-center gap-3 rounded-xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-500"><span className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-lime-300 via-emerald-400 to-emerald-600 text-emerald-950 shadow-[0_12px_24px_-10px_rgb(16_163_127/0.9)] transition group-hover:scale-105 motion-reduce:transition-none"><Leaf className="size-5" /></span><span><span className="block text-lg font-black tracking-tight text-white">EcoSpend</span><span className="block text-[11px] font-medium text-emerald-200">Uang sehat, bumi hemat</span></span></Link>;
}
