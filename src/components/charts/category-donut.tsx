...
export function CategoryDonut({ data, centerLabel = "Pengeluaran" }: { data: CategoryPoint[]; centerLabel?: string }) {
  ...
  return (
    <div className="relative h-64 w-full" role="img" aria-label={`Diagram donat ${centerLabel.toLowerCase()} per kategori`}>
      ...
      <div className="pointer-events-none absolute inset-0 grid place-items-center text-center"><div><p className="text-xs text-slate-500 dark:text-slate-400">{centerLabel}</p><p className="mt-0.5 text-lg font-bold text-slate-950 dark:text-white">{formatRupiah(total, true)}</p></div></div>
    </div>
  );
}
