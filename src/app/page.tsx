      ...
      <main>
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-3">
              {benefits.map((benefit, index) => (
                ...
                <article className="group relative h-full overflow-hidden rounded-[1.6rem] eco-paper p-6 transition duration-300 hover:-translate-y-1.5 hover:border-emerald-700/25 hover:shadow-[0_28px_50px_-28px_rgb(2_44_34/0.35)] motion-reduce:transform-none motion-reduce:transition-none">
                  <div className="pointer-events-none absolute -right-12 -top-12 size-36 rounded-full bg-emerald-200/40 blur-2xl transition group-hover:bg-lime-200/60 dark:bg-emerald-800/15" aria-hidden="true" />
                  <div className="relative flex items-start justify-between gap-4">
                    ...
                    <span className="font-mono text-xs font-ledger text-[#1e6f4e] dark:text-[#8bd3a7]">0{index + 1}</span>
                  </div>
        ...
        <section id="transparansi" className="scroll-mt-24 bg-[var(--paper)] px-5 py-20 sm:px-8 lg:px-10 lg:py-28" aria-labelledby="transparency-title">
          <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-2 lg:gap-20">
            <div className="relative min-h-[430px] overflow-hidden rounded-[2rem] bg-[#edf4e8] p-6 sm:p-10 dark:bg-[#0f2318]">
              <div className="absolute -right-14 -top-14 size-44 rounded-full border-[28px] border-lime-300/30" aria-hidden="true" />
              <div className="relative rounded-2xl border border-emerald-950/10 bg-white p-5 shadow-xl shadow-emerald-950/10 dark:border-white/10 dark:bg-[#0c1a13]">
                <div className="flex items-center gap-3 border-b border-slate-100 pb-4 dark:border-white/10"><span className="grid size-10 place-items-center rounded-xl bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300"><Scale className="size-5" aria-hidden="true" /></span><div><p className="text-xs text-slate-600 dark:text-[#b8c7bd]">Rincian estimasi</p><p className="font-bold text-emerald-950 dark:text-[#eef5ef]">Perjalanan bus kota</p></div></div>
            ...
            <div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:text-[#8bd3a7]">Transparansi metodologi</p>
              <h2 id="transparency-title" className="font-display text-balance text-3xl font-bold text-[#0e3b2c] sm:text-5xl dark:text-[#eef5ef]">Angka yang bisa ditelusuri, batasan yang tidak disembunyikan.</h2>
              <p className="mt-6 text-lg leading-8 text-[#43544b] dark:text-[#b8c7bd]">Setiap estimasi dirancang untuk menyertakan metode, faktor, satuan, sumber, wilayah, dan versi. Anda berhak tahu dari mana sebuah angka berasal.</p>
              <Link href="/methodology" className="mt-7 inline-flex items-center gap-2 font-semibold text-[#1e6f4e] hover:text-[#0e3b2c] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 dark:text-[#8bd3a7] dark:hover:text-[#eef5ef]">
                Pelajari metodologi <ChevronRight className="size-4" aria-hidden="true" />
              </Link>
              <div className="mt-8 rounded-2xl border border-amber-900/15 bg-amber-50 p-5 text-sm leading-6 text-amber-950/80 dark:border-amber-500/30 dark:bg-amber-950/30 dark:text-amber-200/90">
                <strong className="font-semibold text-amber-950 dark:text-amber-200">Penting:</strong> estimasi karbon bersifat indikatif dan sangat dipengaruhi kategori serta kualitas faktor. Bukan pengukuran langsung, audit, sertifikasi, dasar pelaporan regulasi, atau klaim lingkungan.
              </div>
            </div>
          </div>
        </section>

        <section id="keamanan" className="scroll-mt-24 border-y border-[var(--paper-line)] bg-[#f4f7f1] px-5 py-20 sm:px-8 lg:px-10 lg:py-28 dark:bg-[#08130e]" aria-labelledby="security-title">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:text-[#8bd3a7]">Keamanan & privasi</p>
                <h2 id="security-title" className="font-display mt-4 text-balance text-3xl font-bold text-[#0e3b2c] sm:text-5xl dark:text-[#eef5ef]">Data Anda bukan komoditas.</h2>
                <p className="mt-5 leading-7 text-[#43544b] dark:text-[#b8c7bd]">EcoSpend dirancang dengan minimisasi data dan batas akses sebagai prinsip, bukan tambahan di akhir.</p>
              </div>
...
function FeatureBlock({ eyebrow, title, description, bullets, visual, reverse }: FeatureBlockProps) {
  return (
    <article className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
      ...
      <div>
        <p className="inline-flex items-center gap-2 rounded-full border border-[#1e6f4e]/25 bg-[#1e6f4e]/[.07] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[#1e6f4e] dark:border-[#8bd3a7]/25 dark:bg-[#8bd3a7]/10 dark:text-[#8bd3a7]">{eyebrow}</p>
...
function FinanceVisual() {
  return (
    <div className="eco-paper relative overflow-hidden rounded-[1.7rem] p-5 sm:p-8" aria-label="Contoh tampilan keuangan">
      <div className="texture-flow pointer-events-none absolute inset-0 opacity-70" aria-hidden="true" />
      <div className="eco-ledger-row relative flex items-start justify-between gap-4 pb-5"><div><p className="font-ledger text-[11px] uppercase tracking-[0.2em] text-[#1e6f4e] dark:text-[#8bd3a7]">Halaman 01 · Keuangan</p><p className="font-display mt-2 text-3xl font-extrabold text-[#0e3b2c] dark:text-[#eef5ef]">Rp12.460.000</p><p className="mt-1 text-xs text-[#43544b] dark:text-[#b8c7bd]">Saldo bersih jurnal bulan ini</p></div><span className="grid size-11 place-items-center rounded-full bg-[#0e3b2c] text-[#d8ef87]"><WalletCards className="size-5" aria-hidden="true" /></span></div>
      ...
      <div className="relative mt-6"><div className="flex items-end justify-between"><p className="text-sm font-bold text-[#0e3b2c] dark:text-[#eef5ef]">Alur delapan pekan</p><p className="font-ledger text-xs text-[#1e6f4e] dark:text-[#8bd3a7]">+14% bulan ini</p></div><div className="mt-5 flex h-28 items-end gap-2">{[30,55,42,72,48,84,68,92].map((height, i) => <span key={i} className="flex-1 rounded-t-full bg-[#1e6f4e]/15" style={{ height: `${height}%` }}><span className="block w-full rounded-t-full bg-gradient-to-t from-[#1e6f4e] to-[#57c87a]" style={{ height: `${Math.max(20, height - 22)}%` }} /></span>)}</div></div>
    </div>
  );
}
...
function BudgetVisual() {
  ...
  return <div className="eco-paper relative overflow-hidden rounded-[1.7rem] bg-[#f3eee3] p-5 sm:p-8 dark:bg-[#0f2318]" aria-label="Contoh tampilan anggaran"><div className="relative rounded-2xl border border-[#0e3b2c]/10 bg-white p-5 shadow-xl shadow-[#082419]/10 dark:border-white/10 dark:bg-[#0c1a13]"><div className="flex items-center justify-between"><div><p className="font-ledger text-[11px] uppercase tracking-[0.2em] text-[#1e6f4e] dark:text-[#8bd3a7]">Rencana belanja</p><p className="font-display mt-1 text-xl font-extrabold text-[#0e3b2c] dark:text-[#eef5ef]">Rp4,7 jt tersisa</p></div><span className="grid size-11 place-items-center rounded-full bg-[#d99a2b] text-white"><PiggyBank className="size-5" aria-hidden="true" /></span></div><div className="mt-7 space-y-5">{items.map((item) => <div key={item.label}><div className="mb-2 flex justify-between text-xs"><span className="font-semibold text-[#33423a] dark:text-[#d4ded8]">{item.label}</span><span className="font-ledger text-[#2e3d35] dark:text-[#b8c7bd]">{item.spent}</span></div><div className="h-2 overflow-hidden rounded-full bg-[#0e3b2c]/10 dark:bg-white/15"><div className="h-full rounded-full bg-gradient-to-r from-[#1e6f4e] to-[#57c87a]" style={{ width: item.width }} /></div></div>)}</div><div className="mt-7 flex items-center gap-3 rounded-xl bg-[#1e6f4e]/[.07] p-4 dark:bg-emerald-950/60"><Gauge className="size-5 text-[#1e6f4e] dark:text-[#8bd3a7]" aria-hidden="true" /><p className="text-xs leading-5 text-[#0e3b2c] dark:text-[#eef5ef]"><strong>Masih sesuai rencana.</strong><br />Anda telah memakai 58% anggaran bulan ini.</p></div></div></div>;
}

function CarbonVisual() {
  return <div className="relative overflow-hidden rounded-[1.7rem] bg-[#082419] p-5 text-white sm:p-8" aria-label="Contoh tampilan karbon"><div className="texture-flow pointer-events-none absolute inset-0 opacity-30" aria-hidden="true" /><div className="relative rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur"><div className="flex items-center justify-between"><div><p className="font-ledger text-[11px] uppercase tracking-[0.2em] text-[#d8ef87]">Catatan dampak</p><p className="font-display mt-1 text-3xl font-extrabold">86,4 kg CO₂e*</p></div><span className="grid size-11 place-items-center rounded-full bg-[#d8ef87] text-[#082419]"><Leaf className="size-5" aria-hidden="true" /></span></div><div className="mt-7 grid grid-cols-[auto_1fr] items-center gap-6"><div className="grid size-28 place-items-center rounded-full border-[12px] border-white/10 border-t-[#d8ef87]"><span className="font-ledger text-lg font-bold">-12%</span></div><div className="space-y-3"><Legend color="bg-[#d8ef87]" label="Transportasi" value="34%" /><Legend color="bg-[#57c87a]" label="Makanan" value="28%" /><Legend color="bg-[#e3bb68]" label="Belanja" value="21%" /><Legend color="bg-white/40" label="Lainnya" value="17%" /></div></div><div className="mt-7 flex items-center gap-3 border-t border-white/10 pt-5 text-xs text-white/70"><LineChart className="size-4 text-[#d8ef87]" aria-hidden="true" />Estimasi turun dibanding periode sebelumnya.</div></div></div>;
}

function MiniStat({ icon: Icon, label, value, green }: { icon: typeof ArrowRight; label: string; value: string; green?: boolean }) { return <div className="rounded-xl bg-slate-50 p-3 dark:bg-[#14261d]"><span className={`grid size-7 place-items-center rounded-lg ${green ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-300'}`}><Icon className={`size-3.5 ${green ? '-rotate-45' : 'rotate-45'}`} aria-hidden="true" /></span><p className="mt-3 text-[10px] text-slate-600 dark:text-[#b8c7bd]">{label}</p><p className="text-sm font-bold text-slate-900 dark:text-[#eef5ef]">{value}</p></div>; }
...
function DetailRow({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4"><dt className="text-slate-600 dark:text-[#b8c7bd]">{label}</dt><dd className="text-right font-medium text-slate-800 dark:text-[#eef5ef]">{value}</dd></div>; }
function SecurityCard({ icon: Icon, title, text }: { icon: typeof ShieldCheck; title: string; text: string }) { return <article className="group rounded-[1.4rem] border border-[var(--paper-line)] bg-[var(--paper)] p-5 shadow-[0_16px_32px_-26px_rgb(2_44_34/0.5)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_26px_44px_-26px_rgb(2_44_34/0.45)] motion-reduce:transform-none motion-reduce:transition-none"><span className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-emerald-700 to-emerald-500 text-white shadow-md shadow-emerald-900/20 transition group-hover:scale-105 motion-reduce:transition-none"><Icon className="size-5" aria-hidden="true" /></span><h3 className="mt-5 font-bold text-[#0e3b2c] dark:text-[#eef5ef]">{title}</h3><p className="mt-2 text-sm leading-6 text-[#43544b] dark:text-[#b8c7bd]">{text}</p></article>; }
